...
def resource_filepath_function(resource, filename):
    if resource.task:
        ...
        return f'resources/{resource.testcase_as_input.task.assignment.course.lms_id}/{resource.testcase_as_input.task.assignment.lms_id}/{"".join([c for c in resource.testcase_as_input.task.name if c.isalnum()])}/{"".join([c for c in resource.testcase_as_input.description if c.isalnum()])}/inputs/{filename}'
    elif resource.testcase_as_output:
        return f'resources/{resource.testcase_as_output.task.assignment.course.lms_id}/{resource.testcase_as_output.task.assignment.lms_id}/{"".join([c for c in resource.testcase_as_output.task.name if c.isalnum()])}/{"".join([c for c in resource.testcase_as_output.description if c.isalnum()])}/outputs/{filename}'
    else:
